package com.lexsoft.messenger.clent;
...
public class ClientController extends Application {
    ...
    @FXML
    private ComboBox ipAddressCombos = new ComboBox();
    @FXML
    ...
    public void setConnectionWithServer(ActionEvent actionEvent) throws IOException {
        if (!ipAddressCombos.getEditor().getText().isEmpty() & !nickNameTextField.getText().isEmpty()) {
    ...
    private void startClient() throws IOException {
        try {
            Socket server = new Socket(ipAddressCombos.getEditor().getText(), 5000);
